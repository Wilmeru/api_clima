...
function App(){
 ...
 const buscarClima=async()=>{ if(!ciudad.trim()){setError("Escribe una ciudad");return;} try{setCargando(true);setError("");setLugar(null);setClima(null);
        const r1=await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(ciudad)}&count=5&language=es&format=json`); if(!r1.ok)throw new Error("No fue posible buscar la ciudad"); 
        const g=await r1.json(); 
        const resultados=g.results??[]; if(!resultados.length){setError("No se encontró la ciudad");return;}
        const ubicacion=resultados[0]??resultados[1]; setLugar(ubicacion);
